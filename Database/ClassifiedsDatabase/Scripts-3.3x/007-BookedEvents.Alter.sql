...
SELECT	 e.[EventId]
		...
		, c.CategoryAdType
		, (select top 1 Price from dbo.EventTicket where EventId = e.EventId order by price asc) as CheapestTicket
		, (select top 1 Price from dbo.EventTicket where EventId = e.EventId order by price desc) as MostExpensiveTicket
FROM    dbo.[Event] e
