...
"@ >> $outFile;
    ...
    foreach ($fullName in $scriptFiles)
    {
        ...
        {
            $result = Invoke-SqlCmd -Query ("SELECT * FROM ScriptLog WHERE ScriptName = '$($file.Name)' ;") -ServerInstance $DatabaseServer -Database $DatabaseName -U $dbUser -P $dbPassword
            if ($result -ne $null)
            {
...
"@ >> $outFile;
    ...
    else
    {
        ...
        $command = ("sqlcmd.exe -S ") + $DatabaseServer + (" -d ") + $DatabaseName + (" -i ") + $outFile + (" -U ") + $dbUser + (" -P ") + $dbPassword
