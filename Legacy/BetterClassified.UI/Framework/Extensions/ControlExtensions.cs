...
namespace BetterClassified
{
    ...
    {
        public static T FindControl<T>(this Control target, string controlId, bool isRequired = true) where T : class 
        {
            var control = target.FindControl(controlId) as T;
            if (control == null && isRequired)
            {
                throw new NullReferenceException("Control cannot be found within target control.");
            }
            return control;
        }
