...
namespace Paramount.DSL.UI.HttpHanders
{
    public class DocumentHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        protected event EventHandler<HttpHandlerEventArgs> ProcessRequestBegin;

        protected void InvokeProcessRequestBegin(HttpHandlerEventArgs e)
        {
            EventHandler<HttpHandlerEventArgs> handler = ProcessRequestBegin;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            InvokeProcessRequestBegin(new HttpHandlerEventArgs(context, groupingId));
            try
            {
                ...
                if (isServerCaching)
                {
                    stream = ImageCacheController.Get(documentId, width, height, resolution, out isRaw);
                }
                ...
                if (isServerCaching)
                {
                    ImageCacheController.Put(image, documentId, width, height, resolution);
                }
            ...
            {
                EventLog eventLog = new EventLog(ex)
                {
                    SecondaryData = groupingId
                };
                EventLogManager.Log(eventLog);
            }
        }
        ...
        {
            FileInfo rawFile = ImageCacheController.GetRawCacheFile(documentId);
